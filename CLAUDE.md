...
`post_init`; короткая форма `/mt5 <sub>` тоже работает; любой нераспознанный
текст/команда получает краткую подсказку «см. /help» — fallback-хендлер
последним в группе, кнопки/команды срабатывают раньше, молчания не бывает).
Кнопки — двумя
...
margin двигаются при детекте). **Фаза 3 (скриншоты): `/mt5shot [sym]` и кнопка
«📸 График»** — тап/команда без аргумента открывает **меню выбора символа**
(inline: «Все графики» + по кнопке на символ из `Monitor._seen_symbols` —
накопительный набор из orders/positions/deals, символов самих снимков
**и discovery-раунда** `<nonce> ?` («кто жив?»: каждый GRE_Shot отвечает
крошечным `GREalive_<sym>_<nonce>.txt` вместо снимка; `discover_charts()`
гоняется при каждом подключении монитора + по кнопке «🔄 Обновить список»,
callback `m5:charts` — точный матч ДО префиксной ветки `m5:ch:`), т.к. в
pip-пакете НЕТ `charts_get`; `m5:chart` рендерит
меню, `m5:ch:<SYM>`/`m5:ch:*` — съёмка). Раунды снимков/discovery сериализует
`_round_lock` (общий командный «почтовый ящик» и подчистки).
Съёмка — nonce-протокол через `FILE_COMMON\GRE_shot_cmd.txt` (общий
каталог всех терминалов): бот пишет `<nonce> [sym]`, индикатор **GRE_Shot**
(по экземпляру на график, исходник `mt5bot\GRE_Shot.mq5`, эталон — в репо EA)
latch-ит nonce и кладёт `ChartScreenShot()` в свою песочницу
`<terminal>\MQL5\Files\GREshot_<sym>_<nonce>.png` (в Common писать не умеет);
видимость (v1.13): бейдж `OBJ_LABEL "GRE_SHOT_BADGE"` в ПРАВОМ верхнем
углу, ПОД именем EA, которое в этом углу рисует сам терминал
(CORNER/ANCHOR RIGHT_UPPER: y=52 — строкой ниже имени, x=80 — левее шкалы
цен, текст растёт влево; Consolas 9; v1.10 тут стояла вплотную к верху и
наползала на само имя, v1.12 жила в левом стеке дашборда `GRD_*` — по
просьбе владельца вернули в правый угол под имя) —
тикающие часы = опрос жив, после снимка «last: символ время»; `Comment()` НЕ
использовать — он один на график и затёр бы дашборд EA;
`Monitor.take_shots` (в to_thread, лок — только на data_path) опрашивает 0.4 с,
возраст ≥0.3 с, стабильный перерыв 2.0 с (медленный график успевает), таймаут
`shot_timeout_sec`; запись команды — **байтами** (`write_bytes`, с ретраем ×3:
`os.replace` ловит PermissionError в микроокно, пока индикатор держит файл
открытым — MQL5 без FILE_SHARE_DELETE); ⚠️ именно байтами: текстовый режим
Windows дублирует `\r` (CRLF→CRCRLF), хвостовой `\r` утекал в имя PNG на стороне
MQL5 — снимок делался, но glob его не находил (индикатор вдобавок чистит nonce
`StringReplace("\r","")`); не-ASCII символ аргумента ловится как ValueError;
индикатор
командный файл НЕ удаляет («почтовый ящик», гонок нескольких графиков нет),
Python подчищает и команду, и PNG — собранные сразу после `read_bytes`,
«опоздавшие» (ставшие видны на диске ПОСЛЕ stable-break — write-behind/антивирус;
живой случай 16.09: фото терялось) — сперва «second chance» (пауза 1.5 с +
повторный сбор в тот же раунд), затем финальный sweep в конце раунда,
плюс `sweep_shots()` после отправки фото. В боте `_shot_lock` (asyncio.Lock) — один
раунд снимков зараз: двойной тап получает «⏳ уже делается», иначе раунды
съедают чужой nonce/фото. Нюанс: два графика ОДНОГО символа (разные ТФ) дают
одно фото (имя файла без дискриминатора графика; добавить ChartID(), если
понадобится). Фото — НОВЫМИ сообщениями
(`edit_message_text` в фото не превратить → `m5:ch:` отвечает до
`_edit_or_send`). **Питание терминала: `/mt5on` · `/mt5off` (+ ⏻ в пульте
и кнопкой в нижней клавиатуре — она открывает только экран статуса;
`m5:term` → `m5:termoff` → `m5:termoffy`/`m5:termon`)** — санкционированное
исключение из never-launch, только явной командой владельца: выключение —
WM_CLOSE всем видимым окнам процесса (EnumWindows по PID из общего
`_toolhelp_procs`), 15 с на мягкий выход, затем TerminateProcess; включение —
прямой `subprocess.Popen(terminal_path)` (НЕ `mt5.initialize`!). Гейт
`_power_gate`: `allow_trading` + непустой `terminal_path`; выключение — только
через ✅-подтверждение (с числом открытых позиций), включение — сразу.
Монитор переживает оба перехода сам: detach → «📴 недоступен» → re-attach →
«📡 восстановлено» + сделки за простой; авто-гейт `_attach` не тронут.
Фазы 1+3 = чтение + действия над существующими объектами +
снимки;
план и фазы —
...
terminal64.exe, т.к. initialize без пути тоже умеет запускать). Единственное
исключение — питание терминала по явной команде владельца (`/mt5on`):
прямой `Popen`, см. фазу 3 выше; автоматика ничего не запускает никогда.
